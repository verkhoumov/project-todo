...
(function($) {
	...
	var methods = {
		...
		load: function(customOptions) {
			return this.each(function() {
				...
				// Обработка нажатия на кнопку удаления изображения.
				$(this).on('click.TaskDeleteImage', function(e) {
					ajax();
				});
			});
		},
	...
	var success = function() {
		...
		HELPER.noty('success', LANG.imageDeleteSuccess);
	 ...
	 *  Роутер.
	 */
	$.fn.TaskDeleteImage = function(method) {
		if (methods[method]) {
			return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
		} else if (typeof method === 'object' || !method) {
			return methods.load.apply(this, arguments);
		} else {
			$.error('TaskDeleteImage: запрашиваемый метод `' +  method + '` отсутствует!');
		}
	};
})(jQuery);